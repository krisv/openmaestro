...
async def run_flow():
    ...
    agents = [
        PlanningAgent(memory = memory, available_tools = ToolCollection(planningTool, Terminate())),
        Maestro(memory = memory),
    ]
    for agent in agents:
    ...
    try:
        ...
        flow = AlternateFlow(agents=agents, planning_tool=planningTool, active_plan_id=active_plan_id)
