...
class PlanningTool(BaseTool):
    ...
    def _create_plan(
        ...
        plan["step_statuses"][0] = "in_progress"
    ...
    def _mark_step(
        ...
        if step_status == "completed" and step_index < len(plan["steps"])-1:
            plan["step_statuses"][step_index+1] = "in_progress"

        return ToolResult(
            output=f"Step {step_index} updated in plan '{plan_id}'.\n\n{self._format_plan(plan)}"
        )

    def _is_plan_completed(self, plan_id: Optional[str]) -> bool:
        if not plan_id:
